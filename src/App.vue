    ...
    <div :style="'float: left; width:' + width + 'px; margin: 50px 100px; background-color: #FAF9F8; padding: 20px; height: 800px;'" v-chat-scroll="{scrollToTop: false}">
      <div>
        <div v-for="(item, index) in documents" v-bind:key="index">
          <blip-card :photo="photoUri" :position="item.position" :deletable="true" :date="item.date" :on-selected="selected" :hide-options="false" :document="item.document" :on-save="save" :on-deleted="deleted" :editable="false" :on-open-link="selected" :on-unsupported-type="onUnsupportedType"/>
        </div>
      </div>
