<template>
  <div v-if="!isEditing" class="blip-container json-text">
    <div :class="isFailedMessage(status, position)">
      <div
        :class="
          headerText || bodyText || footer
            ? 'bubble ' + position + ' bubble-padding-top-10'
            : 'bubble ' + position
        "
      >
        <bds-button-icon
        ...
        <div class="disable-selection">
          ...
          <div v-if="footerText" class="footer-text">
            <bds-typo
              variant="fs-16"
              bold="regular"
              line-height="plus"
  ...
  <div class="blip-container json-text" v-else>
    <form :class="'bubble ' + position" novalidate v-on:submit.prevent>
      ...
      <bds-button-icon
        ...
        size="short"
        :disabled="errors.any()"
        v-on:click="jsonTextSave(bodyText)"
      ></bds-button-icon>
      <div class="form-group">
        <div>
          <textarea
            @keydown.enter="jsonTextSave($event)"
            type="text"
            name="headerText"
...
export default {
  ...
  methods: {
    init: function() {
      this.headerText = this.document.interactive.header
        ? this.document.interactive.header.text
        : ''
      this.bodyText = this.document.interactive.body
...
.blip-card .fixed-options li div,
.blip-card .fixed-options li span {
  width: auto;
}

.json-text .bubble.bubble-padding-top-10 {
  padding: 10px 0 0 0;
}
