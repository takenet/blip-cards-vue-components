      ...
      <div class="fixed-options" v-if="options">
        <ul>
          <li v-for="(item, index) in options" v-bind:key="index" @click="(editable ? null :select(item))" :class="editable ? '' : ' pointer'">
            <span class="disable-selection" v-html="item.previewText"></span>
          </li>
        </ul>
      </div>
    </div>
  ...
  <div v-else class="blip-container document-select">
    <form v-if="!showOptionDialog" :class="'editing bubble ' + position" novalidate v-on:submit.prevent>
      ...
      <div class="header">
        ...
        <div class="form-group">
          <input type="title" name="title" :class="{'input-error': errors.has('title') }" class="form-control" v-model="title" v-validate="'required'" placeholder="Title" />
          <span v-show="errors.has('title')" class="help input-error">{{ errors.first('title') }}</span>
          <textarea @keydown.enter="documentSelectSave($event)" type="text" name="text" :class="{'input-error': errors.has('text') }" class="form-control textarea" v-model="content" placeholder="Description" />
      ...
      <button v-if="typeof onMetadataEdit === 'function'" class="define-metadata blip-document-select-metadata" @click="editMetadata(fullDocument)">
        {{ metadataButtonText }}
      </button>
    </form>
