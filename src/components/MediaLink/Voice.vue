<template>
    <div>
      ...
      ></bds-button-icon>
      <div class="voice-player-wrapper" v-if="!isEditing">
        <div v-if="simplified">
        ...
        <div v-if="!simplified" class="voice-player-controls">
          <div v-if="!isLoading" class="voice-play-pause">
          ...
          <div v-else class="voice-play-pause">
            <img class="voice-player-loading" :src="loadingGif" alt />
          </div>
          <div class="voice-player-bar">
            <div class="slider" data-direction="horizontal" ref="voiceSlider">
              <div class="progress" ref="voiceProgress">
                <div class="pin" id="progress-pin" data-method="rewind"></div>
              </div>
            </div>
            <input
              class="voice-player-range"
              type="range"
              :value="this.currentTime"
              :max="this.totalTime"
              @input="setVoicePosition($event)"
              @change="setVoicePosition($event)"
            />
        ...
        <form novalidate v-on:submit.prevent>
          ...
          <div class="form-group">
            <input
              ...
              :class="{ 'input-error': errors.has('voice') }"
              v-validate="'required|url'"
            />
            <span v-if="errors.has('voice')" class="help input-error">{{
              errors.first('voice')
            }}</span>
          </div>
          <button
  ...
  export default {
    ...
    mounted: async function() {
      this.voiceUri = isAuthenticatedMediaLink(this.document)
          ? await tryCreateLocalMediaUri(this.document, this.asyncFetchMedia)
          : this.document.uri
      this.initVoice(this.voiceUri)
      this.progress = this.$el.querySelector('.progress')
    },
    destroyed: function() {
      this.voice.removeEventListener('timeupdate', this.voiceTimeUpdated)
      this.voice.removeEventListener('loadedmetadata', this.voiceLoaded)
      this.voice.removeEventListener('ended', this.resetPlay)
      this.voice.addEventListener('canplaythrough', this.voiceReadyToPlay, false)
      this.asyncFetchMedia && URL.revokeObjectURL(this.voiceUri)
    },
    methods: {
      init: async function() {
      ...
      initVoice: function(uri) {
        this.voice = new Audio()
        this.isLoading = true
        this.voice.addEventListener(
          'canplaythrough',
          this.voiceReadyToPlay,
          false
        )
        this.voice.addEventListener('timeupdate', this.voiceTimeUpdated)
        this.voice.addEventListener('loadedmetadata', this.voiceLoaded)
        this.voice.addEventListener('ended', this.resetPlay)
        this.voice.src = uri
        this.voice.load()
      },
      toggleEdit: function() {
      ...
      voiceSave: function() {
        this.$validator.validateAll().then((result) => {
          ...
          this.save({
            ...this.document,
            uri: this.voiceUri,
            type: mime.lookup(this.voiceUri)
              ? mime.lookup(this.voiceUri)
              : 'voice/mp3'
          })
        })
      },
      togglePlay: async function() {
        if (this.isPlaying) {
          ...
          if (this.onVoiceValidateUri) {
            const refreshedVoiceUri = await this.onVoiceValidateUri(this.voiceUri)
            if (refreshedVoiceUri !== this.voiceUri) {
              this.voiceUri = refreshedVoiceUri
              this.initVoice(this.voiceUri)
            }
          }
      ...
      resetPlay: function() {
        if (this.isPlaying && (this.voice.currentTime = this.totalTime)) {
          this.isPlaying = false
          this.voice.pause()
          this.voice.currentTime = 0
        }
      },
      voiceTimeUpdated: function() {
        if (!this.progress) {
          this.progress = this.$el.querySelector('.progress')
        }
  
        try {
          var current = this.voice.currentTime
          var percent = (current / this.totalTime) * 100
          this.progress.style.width = percent + '%'
  
          this.currentTime = this.voice.currentTime
        } catch (e) {}
      },
      voiceLoaded: function() {
        this.totalTime = this.voice.duration
      },
      setVoicePosition: function(event) {
        // srcElement is no supported in FF, but needed if working with IE6-8
        this.voice.currentTime = (event.target || event.srcElement).value
      },
      ...
      changePlaybackRate: function() {
        ...
        this.voice.playbackRate = this.playbackRate
      }
    }
  }
  </script>
